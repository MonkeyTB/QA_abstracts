...
vocab['unk'] = 0
## 1.3 数据预处理
def get_max_length(data,max_len = -1):
    for i in range(len(data)):
        if len(train_x[i]) > max_len:
            max_len = len(train_x[i])
    return max_len
max_len = get_max_length(train_x)
max_len = get_max_length(train_y)
...
# 填充长度
train_x_ids = pad_sequences(train_x_ids,maxlen = max_len,padding = 'post')
train_y_ids = pad_sequences(train_y_ids,maxlen = max_len,padding = 'post')
...
print(char2id)
input_data = pad_sequences([char2id],max_len)
