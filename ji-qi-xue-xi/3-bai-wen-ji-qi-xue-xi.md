...
>
>       > L\_2通常只针对系数矩阵w,而不针对偏置系数b,则损失函数为$$J(w,b)=\frac{1}{2m}\sum_{i=1}^{m}||a^L-y||_2^2+\frac{\lambda}{2m}\sum_{L=2}^L||w||_2^2$$&#x20;
>
>       > 反向传播从:$$w^l=w^l-\alpha\sum_{i=1}^{m}\sigma^{i,l}(a^{x,l-1})^T$$ 变为:$$w^l=w^l-\alpha\sum_{i=1}^{m}\sigma^{i,l}(a^{x,l-1})^T-\alpha\lambda w^l$$&#x20;
